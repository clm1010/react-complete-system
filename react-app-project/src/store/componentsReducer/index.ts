...
export const componentsSlice = createSlice({
	...
	reducers: {
		 ...
		 * @description 添加新组建
		 * @param state 组件列表类型
		 * @param action PayloadAction ComponentInfoType
		 * @description reduxjs/toolkit 2.0 版本, 内置了 immer, 会自动处理不可变数据, 可以不使用 produce
		 */
		 ...
		 * @description 修改组件属性
		 * @param state 组件列表类型
		 * @param action PayloadAction { fe_id: string; newProps: ComponentPropsType }
		 * @description reduxjs/toolkit 2.0 版本, 内置了 immer, 会自动处理不可变数据, 可以不使用 produce
		 */
		...
		/**
		 * @description 隐藏/显示 组件
		 * @param state 组件列表类型
		 * @param action PayloadAction { fe_id: string; isHidden: boolean }
		 * @description reduxjs/toolkit 2.0 版本, 内置了 immer, 会自动处理不可变数据, 可以不使用 produce
		 */
		...
		/**
		 * @description 锁定/解锁 组件
		 * @param state 组件列表类型
		 * @param action PayloadAction { fe_id: string }
		 * @description reduxjs/toolkit 2.0 版本, 内置了 immer, 会自动处理不可变数据, 可以不使用 produce
		 */
		...
		/**
		 * @description 拷贝当前选中的组件
		 * @param state 组件列表类型
		 * @description reduxjs/toolkit 2.0 版本, 内置了 immer, 会自动处理不可变数据, 可以不使用 produce
		 */
		copySelectedComponent: (state: ComponentsStateType) => {
			const { selectedId: copySelectedId, componentList = [] } = state
		...
		selectPrevComponent: (state: ComponentsStateType) => {
			...
			state.selectedId = componentList[selectedIndex - 1].fe_id // 选中上一个, selectedIndex - 1 就是上一个
		},
		...
		selectNextComponent: (state: ComponentsStateType) => {
			...
			state.selectedId = componentList[selectedIndex + 1].fe_id // 选中下一个, selectedIndex + 1 就是下一个
		},
		 ...
		 * @description 修改组件标题
		 * @param state 组件列表类型
		 * @param action PayloadAction { fe_id: string; title: string }
		 * @description reduxjs/toolkit 2.0 版本, 内置了 immer, 会自动处理不可变数据, 可以不使用 produce
		 */
		 ...
		 * @description 移动组件位置
		 * @param state 组件列表类型
		 * @param action PayloadAction { oldIndex: number; newIndex: number }
		 * @description reduxjs/toolkit 2.0 版本, 内置了 immer, 会自动处理不可变数据, 可以不使用 produce
		 */
		moveComponent: (
			state: ComponentsStateType,
